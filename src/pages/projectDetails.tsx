        ...
        <div>
          <div className="flex flex-col lg:gap-10">
            <div className="flex flex-row gap-10 items-end">
              <h4 className="text-start lg:text-[8rem] text-[5rem] sm:text-[6rem] md:text-[7rem] font-medium tracking-tighter project-name">
                {project.title}{" "}
              </h4>
              <Icon
            ...
            <div className="flex flex-col lg:flex-row justify-between lg:items-end items-start gap-10 lg:gap-0 mt-10 lg:mt-0">
              <p className="text-start lg:w-[700px] text-xl lg:text-2xl">
                {project.did}
              </p>
              {project.subtitle === "ferinco" ? (
                ...
                <Button
                  ...
                  link={project.url}
                  id="cap"
                  className="flex items-center gap-2 py-2 px-10 rounded-full bg-transparent border border-black text-black transition-all  duration-300 w-fit font-medium h-fit"
                  iconSize="text-[14px]"
                  target="_blank"
                ></Button>
              )}
            ...
            <div>
              <div className="xl:h-[620px] rounded-[15px] flex flex-col p-0 justify-between w-full relative project-tile mt-10 lg:mt-0">
                ...
                {/* <a href=""> */}
                <div
                  className="rounded-[15px] absolute flex justify-center items-center w-[100%] h-full cursor-pointer bg-black/15 100% transition-all"
                  onClick={() => {
                    setOpenZoom(true);
              ...
              : Images.map((image) => (
                  <div key={image.subtitle}>
                    ...
                    <div className="flex flex-col-reverse gap-5 xl:gap-0 xl:flex-row-reverse justify-between mt-5">
                      <SmallTile
                        image={image.firstMobile}
                        role="B"
                        flexType="flex-col items-end"
                      ></SmallTile>
                      <BigTile
                        image={image.firstBanner}
                        role="A"
                        flexType="flex-col"
                      ></BigTile>
                    </div>
                    <div className="flex flex-col gap-5 xl:gap-0 xl:flex-row justify-between mt-5">
                      <SmallTile
                        image={image.secondMobile}
                        role="C"
                        flexType="flex-col-reverse items-start"
                      ></SmallTile>
                      <BigTile
                        image={image.secondBanner}
                        role="D"
